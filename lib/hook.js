...
// Exported for unit tests — toEvents is the contract between CC's hook
// payload shape and the events.jsonl schema, so it deserves direct coverage.
export function toEvents(kind, data) {
...
// MUST silent-fail at the top level too: if main() rejects (e.g. an unforeseen
// throw inside a try-less branch), an unhandled rejection would surface to CC
// as a hook error popup on every prompt. Catch and exit 0.
//
// BUILDCREW_HOOK_TEST=1 lets the test suite import this file to exercise
// toEvents() without main() blocking on stdin.
if (process.env.BUILDCREW_HOOK_TEST !== "1") {
  main().catch((err) => {
    if (process.env.BUILDCREW_HOOK_DEBUG === "1") {
      try { process.stderr.write(`[buildcrew-hook] main failed: ${err?.message ?? err}\n`); } catch {}
    }
    process.exit(0);
  });
}
